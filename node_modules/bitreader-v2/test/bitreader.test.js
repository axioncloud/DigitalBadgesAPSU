...
var data = Buffer.from('where did you get your _____?');
...
test('BitReader#eat', function (t) {
  t.test('returns the right values after eating', function (t) {
    ...
    t.same(p.eat(1), Buffer.from('w'));
    t.same(p.eat(2), Buffer.from('he'));
    t.same(p.eat(4), Buffer.from('re d'));
    t.end();
  });

  t.test('reading little endian values', function (t) {
    var buf = Buffer.from([0xff, 0x00, 0x00, 0x00]);
  ...
  t.test('error when trying to pass invalid value to endian', function (t) {
    var buf = Buffer.alloc(12); buf.fill(0xff);
    try {
  ...
  t.test('can cast values to ints', function (t) {
    var p;
    p = new BitReader(Buffer.from([0x10, 0x80]));
    t.same(p.eat(2, { integer: true }), 4224);

    p = new BitReader(Buffer.from([0x30, 0x20, 0x10, 0x80]));
    t.same(p.eatInt(4), 807407744);

    p = new BitReader(Buffer.from([0xf6]));
...
test('BitReader#eatBool', function (t) {
  t.same(true, (new BitReader(Buffer.from([0x01])).eatBool()));
  t.same(false, (new BitReader(Buffer.from([0x00])).eatBool()));
  t.end();
});
...
test('BitReader#rewind', function (t) {
  ...
  t.same(p.eat(3), Buffer.from('did'), 'eating 3 should return `did`');
  t.end();
});
...
test('BitReader#peak', function (t) {
  ...
  p.rewind();
  t.same(p.peak(5), Buffer.from('where'));
  t.end();
});

test('BitReader#peak with options', function (t) {
  var p = BitReader(Buffer.from([0xff, 0x00, 0x00, 0x00]));
  var expect = 0xff;
...
test('BitReader#eatString', function (t) {
  var data = Buffer.from('what the who');
  data[4] = 0; data[8] = 0;
  ...
  t.test('includes null byte in offset but not string', function (t) {
    var p = new BitReader(data);
    t.same(p.eatString(), 'what');
    t.same(p.eat(3), Buffer.from('the'));
    t.end();
  });

  test('can specify separator', function (t) {
    var data = Buffer.concat([Buffer.from('hello'), Buffer.from([0xff]), Buffer.from('world')]);
    var p = new BitReader(data);
...
test('BitReader#eatRemaining', function (t) {
  ...
  var data = Buffer.from(string);
  t.test('finish eating the buffer', function (t) {
    var p = new BitReader(data);
    t.same(p.eatRemaining(), Buffer.from(string));
    t.same(p.eatRemaining(), null);
    p.rewind(5);
    t.same(p.eatRemaining(), Buffer.from('that?'));
    t.end();
  });
  ...
  t.test('digest in chunks', function (t) {
    ...
    t.same(chunks[0], Buffer.from('ho'), 'first chunk should match');
    t.end();
  });

  t.test('passing opts to eat', function (t) {
    var data = Buffer.from([0x00, 0xff, 0x00, 0xff, 0x00, 0xff]);
    var p = new BitReader(data);
...
test('BitReader#write', function (t) {
  var string = 'lol';
  var data = Buffer.from(string);
  var moar = Buffer.from('lercoaster');

  t.test('appends another buffer to the internal buffer', function (t) {
    var p = new BitReader();
    t.same(p.getBuffer(), Buffer.from(''));
    t.same(p.write(data).getBuffer(), Buffer.from('lol'));
    t.same(p.write(moar).getBuffer(), Buffer.from('lollercoaster'));
    t.end();
  });
...
test('BitReader#remaining', function (t) {
  var data = Buffer.from('lollerskates');
  var p = new BitReader(data);
