...
InternationalText.prototype.length = function () {
  return (
    (this.keyword.length + 1)
      + 1 // compressed
      + 1 // compressionMethod
      + (Buffer.from(this.languageTag||'').length + 1)
      + (Buffer.from(this.translatedKeyword||'').length + 1)
      + (this.compressed ? this.compressedText.length : Buffer.from(this.text||'').length)
  );
};
