...
var BitWriter = require('bitwriter-v2');
...
function crc(type, data) {
  if (data._buffer) { data = data._buffer }
...
Output.prototype.done = function done(callback) {
  var hex = this.data._buffer.length.toString(16);
  var a = parseInt(hex.slice(-2), 16);
  var b = parseInt(hex.slice(-4, -2), 16);
  var c = parseInt(hex.slice(-6, -4), 16);
  var d = parseInt(hex.slice(-8, -6), 16);
